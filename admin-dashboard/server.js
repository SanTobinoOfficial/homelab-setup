...
app.get("/api/metrics", async (req, res) => {
  try {
    const r = await agentRequest("/api/metrics");
    res.json(r.data);
  } catch (e) {
...
app.get("/api/services", async (req, res) => {
  try {
    const r = await agentRequest("/api/services");
    res.json(r.data);
  } catch (e) {
    res.status(503).json({ error: e.message });
  }
...
app.get("/api/logs/:service", async (req, res) => {
  try {
    const r = await agentRequest(`/api/logs/${req.params.service}`);
    res.json(r.data);
  } catch (e) {
    res.status(503).json({ error: e.message });
  }
...
app.post("/api/services/restart", async (req, res) => {
  try {
    const r = await agentRequest("/api/services/restart", "POST", req.body);
    res.json(r.data);
  } catch (e) {
    res.status(503).json({ error: e.message });
  }
...
app.post("/api/services/stop", async (req, res) => {
  try {
    const r = await agentRequest("/api/services/stop", "POST", req.body);
    res.json(r.data);
  } catch (e) {
    res.status(503).json({ error: e.message });
  }
...
app.get("/api/storage", async (req, res) => {
  try {
    const r = await agentRequest("/api/storage");
    res.json(r.data);
  } catch (e) {
    res.status(503).json({ error: e.message });
  }
...
app.get("/api/config", (req, res) => {
  res.json({ laptop_ip: LAPTOP_IP, agent_url: AGENT_URL });
});
...
app.listen(PORT, () => {
  ...
  console.log(` API Key:   ${AGENT_API_KEY ? "configured" : "MISSING - check .env"}\n`);
});
